function cost = costPK_CLP(opt, model, dose_schedule_qsp, tindex, expData, V, opt_avg)
...
diff = log(simData.Data(:,1).*MW./K_B) - log(expData);
cost = norm(diff)^2 + sum(abs(.4.*(opt-opt_avg)'*V));
